...
color_category_df <- data.frame(color_category) #create data frame
color_category_df <- left_join(color_category_df, color_df, by="color_category") #join colors

# Save color_category to csv
write.csv(color_category_df, file="../data/preprocessed/color_category.csv", row.names = FALSE)
